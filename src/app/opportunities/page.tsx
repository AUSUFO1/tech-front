...
export default async function Page({
  ...
  const [latestOpportunities, {latestNews}] = await Promise.all([getOpportunitiesContent(), getNewsContent()])
  const params = (await searchParams) ?? {}
  const category = params.category
  const currentPage = getCurrentPage(params.page)
  const filteredOpportunities = latestOpportunities.filter((item) => {
    if (!category) return true
    if (category === 'scholarships') return item.opportunityType.toLowerCase() === 'scholarship'
    if (category === 'bootcamps') return item.opportunityType.toLowerCase() === 'bootcamp'
    return true
  })
  const paginated = paginateItems(filteredOpportunities, currentPage)
  ...
  return (
    <main className="mx-auto flex w-full max-w-[1360px] flex-col px-5 pb-12 sm:px-8 lg:px-16 lg:pb-16">
      <section className="border-b border-border py-8 lg:py-10">
        <h1 className="font-display text-[2.9rem] font-bold leading-[0.92] tracking-[-0.07em] text-primary-text sm:text-[4.4rem]">
          Opportunities
        </h1>
      </section>

      <SectionSearchBar placeholder="Search scholarships, fellowships, grants, and bootcamps..." />
