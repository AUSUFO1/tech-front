...
import {latestNews, latestOpportunities} from '@/lib/mock-content'
          ...
          {filteredOpportunities.map((item) => (
            <article key={item._id} className="border-b border-border py-5">
              ...
              <div className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-2 text-[0.68rem] font-bold uppercase tracking-[0.14em] text-muted-text">
                <span className="bg-primary-green px-2.5 py-1 text-white">{item.opportunityType}</span>
                <span>{item.location}</span>
                <span>{formatViews(item.views)}</span>
              </div>
            </article>
          ))}
          <StandardPagination summary={`1-${filteredOpportunities.length} of ${filteredOpportunities.length}`} />
        </div>
